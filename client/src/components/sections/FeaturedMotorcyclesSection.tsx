/**
 * RIDDY Featured Motorcycles Section - Filtrada por Estado
 * Layout split: carrosséis à esquerda + mapa com pins à direita
 * Espelho da FeaturedCarsSection mas para motos
 */
...
// ─── MotorcycleCard ──────────────────────────────────────────────────────────
function MotorcycleCard({ vehicle, highlighted = false }: { vehicle: Vehicle; highlighted?: boolean }) {
  ...
  // Clean duplicate brand in model name
  ...
  return (
    <Link href={`/motorcycles/${vehicle.id}`}>
      <motion.div
        whileHover={{ y: -3, scale: 1.01 }}
        transition={{ duration: 0.2 }}
        className={`w-[calc(50vw-24px)] sm:w-[220px] md:w-[240px] lg:w-[260px] max-w-[260px] group cursor-pointer flex-shrink-0 rounded-xl overflow-hidden bg-[#1A2235] border border-white/8 hover:border-orange-500/30 transition-all duration-200 shadow-md hover:shadow-orange-500/10 ${
          highlighted ? "ring-2 ring-orange-500/60" : ""
        }`}
      >
        ...
        <div className="p-3">
          ...
          <div className="flex items-center gap-1.5 mb-2">
            ...
            {rating > 0 ? (
              ...
              <span className="text-orange-500 text-[10px] font-medium">Novo</span>
            )}
          </div>
    ...
    <div className="mb-8 sm:mb-10 md:mb-12">
      <div className="flex items-center justify-between mb-4 sm:mb-6 px-4 sm:px-0">
        <div className="flex items-center gap-3">
          ...
          {cityLink && (
            <Link href={cityLink}>
              <span className="text-orange-400 hover:text-orange-300 text-sm hidden sm:inline">
                Ver todas →
              </span>
            </Link>
          )}
      ...
      >
        {vehicles.map((vehicle) => (
          <div key={vehicle.id} className="snap-start">
            <MotorcycleCard vehicle={vehicle} highlighted={vehicle.id === highlightedId} />
          </div>
        ))}
      </div>
    </div>
          ...
          >
            {availableStates.length === 0 ? (
              <p className="text-gray-400 text-sm p-4 text-center">
                Nenhum estado com motos disponíveis
              </p>
            ) : (
              availableStates.map((s) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 hover:bg-white/5 text-left transition-colors ${
                    s.code === currentCode ? "text-orange-400" : "text-white"
                  }`}
                >
                  <span className="text-sm">{s.name}</span>
                  <span className="text-xs text-gray-400">
                    {s.vehicleCount} moto{s.vehicleCount !== 1 ? "s" : ""}
                  </span>
                </button>
              ))
            )}
  ...
  return (
    <div className="text-center py-16">
      ...
      <h3 className="text-xl font-semibold text-white mb-2">
        {stateName
          ? `Nenhuma moto disponível em ${stateName}`
          : "Nenhuma moto cadastrada ainda"}
      </h3>
      <p className="text-gray-400 mb-6">
        {stateName
          ? "Seja o primeiro anfitrião de motos neste estado!"
          : "Seja o primeiro a anunciar sua moto na RIDDY!"}
      </p>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function FeaturedMotorcyclesSection() {
  ...
  // Buscar estados disponíveis com motos
  const statesQuery = trpc.vehicle.getStates.useQuery(
    { vehicleType: "motorcycle" },
    { staleTime: 5 * 60 * 1000 }
  );

  // Buscar motos agrupadas por cidade no estado do usuário (top 2 cidades)
  const vehiclesQuery = trpc.vehicle.getGroupedByCityInState.useQuery(
    { state: userState?.code ?? "SP", limit: 10, maxCities: 2, vehicleType: "motorcycle" },
    {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 sm:mb-8 px-4 sm:px-0">
          <div>
            <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-white">
              Motos disponíveis
              {userState && (
                <span className="text-orange-400"> em {userState.name}</span>
              )}
            </h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-lg mb-4">
              Selecione seu estado para ver motos disponíveis
            </p>
            <StatePicker
              currentCode={null}
              availableStates={availableStates}
              ...
              {cities.map((city) => {
                ...
                return (
                  <MotorcycleCarousel
                    ...
                    cityLink={`/motorcycles?city=${encodeURIComponent(city)}&state=${userState.code}`}
